...
public class ShareFileUtils {
     ...
     * @param path
     */
    public static void shareUrl(Context context, String path, String title) {
    ...
    public static void shareImageToQZone(Context context, String path) {
        if (!isAppInstall(context, "com.qzone")) {
            Toast.makeText(context, "您还没有安装QQ空间", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public static void shareImageToWeibo(Context context, String path) {
        if (!isAppInstall(context, "com.sina.weibo")) {
            Toast.makeText(context, "您还没有安装新浪微博", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private static void shareImage(Context context, String path, List<String> pathList, String pkg, String cls) {
        if (path == null && pathList == null) {
            Toast.makeText(context, "找不到您要分享的图片文件", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            if (path != null) {
                //单张图片
                if (path.isEmpty()) {
                    Toast.makeText(context, "图片不存在，请检查后重试", Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                for (int i = 0; i < pathList.size(); i++) {
                    if (pathList.get(i).isEmpty()) {
                        ToastUtils.s(context, "第" + (i + 1) + "张图片不存在，请检查后重试");
                        return;
                    }
            ...
            Toast.makeText(context, "分享失败，未知错误", Toast.LENGTH_LONG).show();
        }
    }
